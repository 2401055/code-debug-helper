...
export default function Learn() {
  ...
  const { data: progress } = trpc.auth.getProgress.useQuery(undefined, {
    enabled: !!user,
  });

  const updateProgress = trpc.auth.updateProgress.useMutation({
    onSuccess: () => {
      utils.auth.getProgress.invalidate();
  ...
  const isCompleted = useCallback(
    (categoryId: string) => {
      return progress?.some((p) => p.errorCategoryId === categoryId && p.completed);
    },
    [progress]
  );
        ...
        {errorCategories.map((cat) => {
          ...
          return (
              ...
              >
                ...
                />
                {isCompleted(cat.id) && (
                  <CheckCircle2 className="h-4 w-4 text-success absolute top-3 right-3" />
                )}
                    ...
                    {user && (
                      <Button
                        ...
                        className={`h-7 text-[10px] gap-1.5 ${
                          isCompleted(error.category)
                            ? "text-success hover:text-success hover:bg-success/10"
                            : ""
                        }`}
                        onClick={() =>
                          updateProgress.mutate({ errorCategoryId: error.category })
                        }
                      ...
                      >
                        ...
                        {isCompleted(error.category) ? "Completed" : "Mark as Completed"}
                      </Button>
                    )}
                  </div>
