...
export default function Editor() {
  ...
  const saveSnippet = trpc.code.saveSnippet.useMutation({
    onSuccess: () => {
      setIsSaving(false);
  ...
  const handleSave = useCallback(() => {
    ...
    setIsSaving(true);
    saveSnippet.mutate({
      title: `${language.charAt(0).toUpperCase() + language.slice(1)} Snippet - ${new Date().toLocaleTimeString()}`,
      code: code.trim(),
      language,
      errorMessage: errorMessage.trim() || undefined,
      aiAnalysis: analysis ? JSON.stringify(analysis) : undefined,
      isFixed: analysis ? analysis.severity !== "error" : false,
    });
  }, [code, language, errorMessage, analysis, saveSnippet]);
