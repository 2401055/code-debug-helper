...
export const appRouter = router({
  ...
  auth: router({
    ...
    updateProgress: publicProcedure
      .input(z.object({ errorCategoryId: z.string() }))
      .mutation(async ({ ctx, input }) => {
        if (!ctx.user) throw new Error("Unauthorized");
        await db.markProgressCompleted(ctx.user.id, input.errorCategoryId);
        return { success: true };
      }),
  ...
  code: router({
    getSnippets: publicProcedure.query(async ({ ctx }) => {
      if (!ctx.user) return [];
      return await db.getCodeSnippetsByUser(ctx.user.id);
    }),
    ...
    deleteSnippet: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if (!ctx.user) throw new Error("Unauthorized");
        await db.deleteCodeSnippet(input.id, ctx.user.id);
        return { success: true };
      }),
